...
X_train = pad_sequences(
    train_comment_sequences, maxlen=MAX_LENGTH, padding=PADDING, truncating=TRUNCATE
)
X_val = pad_sequences(
    val_comment_sequences, maxlen=MAX_LENGTH, padding=PADDING, truncating=TRUNCATE
)
X_test = pad_sequences(
    test_comment_sequences, maxlen=MAX_LENGTH, padding=PADDING, truncating=TRUNCATE
)
...
x = Embedding(input_dim=VOCAB_SIZE + 1, output_dim=EMBED_SIZE, mask_zero=True)(
    sequence_input
)
...
model.fit(
    ...
    epochs=EPOCHS,
    callbacks=[
        RoCAUCEvalCallback((X_val, y_val)),
        tf.keras.callbacks.ModelCheckpoint(
            filepath=checkpoints_path, save_weights_only=False
        ),
        tf.keras.callbacks.ModelCheckpoint(
            filepath=best_model, save_best_only=True, save_weights_only=False
        ),
    ],
)
